...
public class PoseEstimator extends SubsystemBase {
    ...
    private final ShuffleboardTab tab = Shuffleboard.getTab("Odometry Data");
    private final GenericEntry xPoseDiffEntry = tab.add("XOdom Diff", 0).getEntry();
    private final GenericEntry yPoseDiffEntry = tab.add("YODom Diff", 0).getEntry();
    private final GenericEntry totalDiffEntry = tab.add("totalDiff", 0).getEntry();
    private final GenericEntry rToSpeaker = tab.add("Distance to Speaker", 0).getEntry();
    ...
    public boolean isEstimateReady(Pose2d pose) {
        ...
        // visionAccurate method sees if Apriltags present in Vision.java
        return Vision.getInstance().visionAccurate(pose);
    }
